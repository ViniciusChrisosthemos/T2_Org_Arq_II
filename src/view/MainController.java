...
public class MainController implements Initializable {
	...
	private void setEventHandlers() {
		btnConfigCache.setOnAction(action -> setCacheConfig(false));
		...
		btnLoadCacheConfigFile.setOnAction(action -> setCacheConfig(true));
	...
	private void setCacheConfig(boolean file) {
		try {
			boolean setup = false;
			int cacheSize,blockAmount,wordSize,ways;
			if(file){
				File f = showFileDialog();
				setup = manager.loadCacheConfig(f);

				lblCacheConfigFile.setTextFill(Color.GREEN);
				lblCacheConfigFile.setText(f.getName());
			}else{
				cacheSize = Integer.parseInt(txtCacheSize.getText());
				blockAmount = Integer.parseInt(txtWordAmount.getText());
				wordSize = Integer.parseInt(txtWordSize.getText());
				ways = Integer.parseInt(txtWays.getText());
				setup = manager.setCacheConfig(cacheSize, blockAmount, wordSize, ways);
			}
			...
			if(setup){
				...
				lblCacheStatus.setTextFill(Color.GREEN);
				lblCacheStatus.setText("Configurada!");	
				Console.log("Cache configurada com sucesso!");
			}else{
				lblLines.setText("0");
				lblSetAmount.setText("0");
				String addrFormat = "[ Tag = - , Conjunto = - , Bloco = - ]";
				lblAddrFormat.setText(addrFormat);
				lblCacheStatus.setTextFill(Color.RED);
				lblCacheStatus.setText("Valores inválidos!");
				Console.log("Valores inválidos para a cache.");
			}			
	...
	private File showFileDialog() throws Exception {
		...
		// Filtra os arquivos para mostrar somente os com extensão .txt
